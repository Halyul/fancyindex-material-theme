if (!!(window.history && history.pushState)) {
    ...
    setTimeout(function () {
      ...
      for (var i = 0; i < segments.length; i++) {
        if (segments[i] !== '') {
          ...
          currentPath += '';
          breadcrumbs += '&emsp;<a href="' + currentPath + '" class=\"breadcrumb\">Root<\/a>';
        }
      }
  ...
  var swapPage = function(href) {
    ...
    req.send(null);
    if (req.status == 200) {
      var target = document.getElementsByClassName('card-panel')[0];
      var div = document.createElement('div');
      div.innerHTML = req.responseText;
      var elements = div.getElementsByClassName('card-panel')[0];
